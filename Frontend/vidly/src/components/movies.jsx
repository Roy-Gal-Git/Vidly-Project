  ...
  render() {
    ...
    <React.Fragment>
      <main className='container mt-4'>
        <div className='mb-3'>{(this.state.movies.length > 0) ? `Showing ${this.state.movies.length} movies in the database.` : `There are no movies in the database.`}</div>
        <table id='movies' className='table table-hover'>
          <tbody>
            <tr>{(this.state.movies.length > 0) ? this.renderTableHeader() : null}</tr>
